...
public class CreateUserTest extends UserBaseClassDummy {
    ...
    void testCreateUserWithValidDataInAllAvailableFields(UserModel userBody) throws InterruptedException {
        OffsetDateTime beforeRequest = OffsetDateTime.now(ZoneOffset.UTC).truncatedTo(ChronoUnit.MILLIS).minusSeconds(1);
        UserModel userRsp = restWrapper.usingUsers().createItem(userBody);
        OffsetDateTime afterRequest = OffsetDateTime.now(ZoneOffset.UTC).truncatedTo(ChronoUnit.MILLIS).plusSeconds(3);
